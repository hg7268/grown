			...
			<div className="rounded-[60px] bg-white mx-auto p-[0.35rem] px-[12px] shadow-lg  2xl:w-[1195px] xl:w-[1100px] w-[95%]">
				...
				<div className={`overflow-hidden duration-300 ease-in-out ${openMobileMenu ? "h-56 sm:h-36 md:h-10" : "h-0"}`}>
					<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 justify-between px-4 mt-3">
						{["Home", "Features", "Pricing", "Partner", "Support", "Contact"].map((link, i) => (
							<a
								...
								className={`hover:text-[#111111] ${activeLink === link ? "text-[#111111]" : "text-[#666666]"} ${i % 2 == 0 ? "text-left" : "text-right"} sm:text-center`}
							>
								{link}
							</a>
						))}
					...
					<div className="flex flex-col sm:flex-row sm:px-10 md:hidden">
						...
						<button className="sm:ms-auto rounded-[32px] px-[20px] py-3 mt-3 bg-gradient-to-b from-[#555555] to-[#000000] text-white">Start For Free</button>
					</div>
				</div>
			</div>
		</header>
